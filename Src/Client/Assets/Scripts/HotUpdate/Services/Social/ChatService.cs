...
namespace Services
{
    ...
    class ChatService : Singleton<ChatService>, IDisposable, IInitializable
    {
        ...
        void OnChat(object sender, ChatResponse message)
        {
            ...
            {
                ChatManager.Instance.AddMessage(ChatChannel.Local, message.LocalMessages.ToList());
                ChatManager.Instance.AddMessage(ChatChannel.World, message.WorldMessages.ToList());
                ChatManager.Instance.AddMessage(ChatChannel.System, message.SystemMessages.ToList());
                ChatManager.Instance.AddMessage(ChatChannel.Private, message.PrivateMessages.ToList());
                ChatManager.Instance.AddMessage(ChatChannel.Guild, message.GuildMessages.ToList());
                ChatManager.Instance.AddMessage(ChatChannel.Team, message.TeamMessages.ToList());
            }
            else
