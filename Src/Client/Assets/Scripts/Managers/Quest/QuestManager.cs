...
namespace Managers
{
    ...
    class QuestManager : Singleton<QuestManager>
    {
                ...
                if (kv.Value.PreQuest > 0)
                {
                    ...
                    if (allQuests.TryGetValue(kv.Value.PreQuest, out newQuest))//获取前置任务
                    {
                        if (newQuest.info == null)
                            continue;//没接
                        if (newQuest.info.Status != QuestStatus.QuestFailed)
                            continue;//没完成
                    }
                    else
                    {
        ...
        private void AddNpcQuest(int npcId, Quest quest)
        {
            ...
            {
                if (npcId == quest.define.SubmitNPC && quest.info.Status == QuestStatus.QuestCompleted)
                {
                    if (!this.npcQuests[npcId][NPCQuestStatus.Complete].Contains(quest))
                        this.npcQuests[npcId][NPCQuestStatus.Complete].Add(quest);
                }
                else if (npcId == quest.define.SubmitNPC && quest.info.Status == QuestStatus.QuestInProgress)
                {
        ...
        {
            if (quest.info == null || quest.info.Status == QuestStatus.QuestCompleted)
            {
                //接受任务或提交任务
            ...
            else if (quest.info != null || quest.info.Status == QuestStatus.QuestCompleted)
            {
                //未完成的任务
        ...
        void OnQuestDialogClose(UIWindow sender, UIWindow.WindowResult result)
        {
            ...
            if (result == UIWindow.WindowResult.Yes)
            {
                ...
                else if(dlg.quest.info.Status == QuestStatus.QuestCompleted)
                {
                    //提交任务
